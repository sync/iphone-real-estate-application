...
- (void)drawRect:(CGRect)rect {
	...
	// Draw bottom image
	[[(ozEstateAppDelegate *)[[UIApplication sharedApplication]delegate] cachedImage:@"paper.png"] drawInRect:CGRectMake(self.bounds.origin.x + 10.0, self.bounds.origin.y + 10.0, 115.0, 100.0) blendMode:kCGBlendModeNormal alpha:1.0];
	...
	if (self.title) {
		[priceColor set];
		[self.title drawInRect:CGRectMake(self.bounds.origin.x + 10 + 15.0, self.bounds.origin.y + 12.0 + 10.0, 85.0, 16.0 + 5.0) withFont:boldFont lineBreakMode:UILineBreakModeTailTruncation alignment:UITextAlignmentRight];
		
	}
	
	if (self.subtitle) {
		[textColor set];
		[self.subtitle drawInRect:CGRectMake(self.bounds.origin.x + 10 + 5.0, self.bounds.origin.y + 13.0 + 13.0  + 10.0, 100.0, 54.0) withFont:boldFont lineBreakMode:UILineBreakModeTailTruncation alignment:UITextAlignmentLeft];
